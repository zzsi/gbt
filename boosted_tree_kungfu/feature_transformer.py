...
class FeatureTransformer:
    ...
    def __init__(
        ...
        add_categorical_stats: bool, default False
            If True, add derived features each categorical variable, computed as mean and count (and other stats) of the target value per level.
        ...
        # TODO: not being able save preprocess_fn, postprocess_fn can be problem if these functions change frequently.
    ...
    def derived_features(self):
        if self.add_categorical_stats:
            return [
                self.derived_mean_feature_name(f)
                for f in self.categorical_features
            ] + [
                self.derived_count_feature_name(f)
                for f in self.categorical_features
            ]
        else:
            return []
    ...
    def transform(
        ...
        ):  # check if its training and calculate cv object
        ...
        if (
            self.apply_vectorize and not self.count_vectorize
            ...
            for a in list(self.apply_vectorize.vocabulary_):
                features.append(a)
            self.features = (
                features  # initializing so it could be used in testing
            )
    ...
    def count_vectorize_merge(self, count_vector, df, cv):
        ...
        data_frame = pd.DataFrame(
            count_vector.toarray(), columns=list(cv.vocabulary_)
        )
    ...
    def convert_categorical_features(self, df):
        for f in self.categorical_features:
            if self.order_categoricals:
                ...
                df[f].cat.set_categories(self.cats[f], inplace=True)
