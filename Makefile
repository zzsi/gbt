...
check-safety:
	...
	$(BANDIT_COMMAND_FLAG)poetry run bandit -ll -r boosted_tree_kungfu/

.PHONY: check-style
check-style:
	$(BLACK_COMMAND_FLAG)poetry run black --config pyproject.toml --check ./
...
#! 	poetry run pylint boosted_tree_kungfu tests/**/*.py
